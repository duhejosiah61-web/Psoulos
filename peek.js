...
export function usePeek(charactersRef, activeProfileRef, soulLinkMessagesRef, soulLinkGroupsRef) {
    ...
    const peekLocked = ref(true); // Default to true to show FaceID animation
    const peekFaceIdSuccess = ref(false); // To trigger the success tick animation
...
`;
    ...
    const unlockPeek = () => {
        ...
        if (input === String(peekEffectivePasscode.value || '')) {
            // Trigger FaceID unlocking sequence
            peekLocked.value = true;
            peekFaceIdSuccess.value = false;
            setTimeout(() => {
                peekFaceIdSuccess.value = true;
                setTimeout(() => {
    ...
    onMounted(async () => {
        ...
        peekDark.value = true; // Always true for high-fidelity UI
