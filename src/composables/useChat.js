...
import { callAI, buildChatCompletionUrlCandidates } from '../js/api.js';
...
export function useChat(
    ...
    const triggerSoulLinkAiReply = async (options = {}) => {
        ...
        try {
            ...
            let response = null;
            let lastErr = null;
            const maxTokens = (typeof profile.max_tokens === 'number') ? profile.max_tokens : 2000;
            ...
            for (const url of urls) {
                try {
                    ...
                    if (!res.ok) {
                        ...
                        lastErr = new Error(`接口返回状态码 ${res.status}: ${errText.slice(0, 200)}`);
...
4. 必须在回复开头明确写出你是哪位成员，格式为：成员名: 正式回复内容`;
    ...
    const displayChatCharactersByTag = computed(() => {
        const groups = {};
        const list = (characters.value || []).filter(Boolean);
        list.forEach(char => {
            let tags = char.tags;
            ...
            tags.forEach(tag => {
                // If activeChatTag is selected, only show that tag
                if (activeChatTag.value && tag !== activeChatTag.value && activeChatTag.value !== '') return;
        ...
        const result = Object.keys(groups).map(tag => {
            let maxTimestamp = 0;
            groups[tag].forEach(char => {
                const ts = getLastMsgTimestamp(char.id, false);
                if (ts > maxTimestamp) maxTimestamp = ts;
            });
            return { tag, characters: groups[tag], lastMsgTimestamp: maxTimestamp };
        });

        result.sort((a, b) => {
            if (b.lastMsgTimestamp !== a.lastMsgTimestamp) {
    ...
    const chatCollapsedTags = ref(new Set());
    const toggleChatTagCollapse = (tag) => {
        if (chatCollapsedTags.value.has(tag)) {
            chatCollapsedTags.value.delete(tag);
        } else {
            chatCollapsedTags.value.add(tag);
        }
        chatCollapsedTags.value = new Set(chatCollapsedTags.value);
    };
    const isChatTagCollapsed = (tag) => chatCollapsedTags.value.has(tag);
    ...
    const formatRightnowText = (text) => {
        ...
        if (window.DOMPurify) {
            ...
            // Fallback if DOMPurify failed to load: escape all HTML to avoid XSS
            return `<div style="word-break: break-word; line-height: 1.8;">${text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")}</div>`;
        }
        return outputHtml;
    };
    ...
    return {
        ...
        totalUnrepliedCount, activeChatTag, chatTags, displayChatCharacters, displayChatCharactersByTag, displayChatGroups,
