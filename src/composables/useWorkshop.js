...
export function useWorkshop({
  ...
  const collapsedTags = ref(new Set());
  ...
  const charactersByTag = computed(() => {
    const groups = {};
  ...
  const toggleTagCollapse = (tag) => {
    ...
    collapsedTags.value = new Set(collapsedTags.value);
  };

  const isTagCollapsed = (tag) => collapsedTags.value.has(tag);
  ...
  const toggleGlobalWorldbook = (wb) => {
    wb.globalEnabled = !wb.globalEnabled;
    saveWorldbooks();
  };
  ...
  return {
    characters, charactersByTag, worldbooks, presets,
    ...
    collapsedTags, toggleTagCollapse, isTagCollapsed,
